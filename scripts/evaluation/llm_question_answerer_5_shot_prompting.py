...
class QuestionAnswerer:
    def __init__(self, use_few_shot: bool = False):
        ...
        suffix = "_5shot" if self.use_few_shot else ""
        self.results_csv = f"llm_test_results_{self.model.replace('.', '_').replace('-', '_')}{suffix}.csv"
    ...
    def save_result_to_csv(self, question_id, correct_answer, llm_answer, shots_used: int = 0):
    ...
    def get_examples_for_subdomain(self, df, target_subdomain, exclude_question_id, num_examples: int = 5):
        """Return up to num_examples formatted QA examples from the same subdomain, excluding the current question."""
        try:
            same_sub = df[(df['subdomain_name'] == target_subdomain) & (df['Unique_Serial'] != exclude_question_id)]
        except Exception:
            return []
        if same_sub.empty:
            return []
        sample = same_sub.sample(n=min(num_examples, len(same_sub)))
        examples: list[str] = []
        for _, r in sample.iterrows():
            opts = self.parse_options(r.get('options', ''))
            if len(opts) < 4:
                continue
            q = r.get('question', '')
            ans = str(r.get('correct_answer', '')).upper()
            example = (
                f"Question: {q}\n\n"
                f"A) {opts[0]}\nB) {opts[1]}\nC) {opts[2]}\nD) {opts[3]}\n\n"
                f"Answer: {ans}\n\n---"
    ...
    def format_question_for_llm(self, question, options, examples=None):
        """Format question for the LLM using 5-shot prompting when examples are provided."""
        ...
        parts = []
        if examples:
            parts.append("Here are some examples from the same subdomain. Study the pattern and answer similarly:")
            parts.append("")
            parts.extend(examples)
            parts.append("Now answer this question:")
            parts.append("")
        else:
            parts.append("Answer this multiple-choice question. Respond only with A, B, C, or D.")
            parts.append("")

        parts.append(f"Question: {question}")
        parts.append("")
        parts.append(f"A) {options[0]}")
        parts.append(f"B) {options[1]}")
        parts.append(f"C) {options[2]}")
        parts.append(f"D) {options[3]}")
        parts.append("")
        parts.append("Answer:")
        return "\n".join(parts)
    
    def ask_llm(self, question, options, examples=None, max_retries=3):
        """Ask the LLM to answer the question, optionally including few-shot examples."""
        formatted_question = self.format_question_for_llm(question, options, examples)
    ...
    def test_single_question(self, df, index=None, show_details=True, skip_processed=True):
        """Test a single question with 5-shot prompting from the same subdomain when possible."""
        if index is None:
        ...
        # Build few-shot examples from the same subdomain (only if enabled globally)
        examples = self.get_examples_for_subdomain(df, subdomain, question_id, num_examples=5) if self.use_few_shot else []
        if show_details:
            if examples:
                print(f"\n🎯 Using {len(examples)} examples from '{subdomain}' for 5-shot prompting")
            else:
                print(f"\nℹ️ No examples available for subdomain '{subdomain}', using zero-shot")
            print(f"\n🤖 Asking {self.model}...")
        
        # Ask LLM with examples
        llm_answer = self.ask_llm(question, options, examples)
        
        # Save result to CSV immediately
        self.save_result_to_csv(question_id, correct_answer, llm_answer, shots_used=len(examples))
...
def main():
    ...
    # Ask once whether to use 5-shot prompting; this also controls CSV suffix
    use_few_shot_global = input("Use 5-shot prompting (examples from same subdomain)? (y/N): ").strip().lower() == 'y'
    answerer = QuestionAnswerer(use_few_shot=use_few_shot_global)
